...
describe('ConclusionScope — observer/observed identity', () => {
    it('stores observer and observed from constructor', () => {
        ...
        const scope = new ConclusionScope(http, 'ws-test', 'agent-prime', 'owner');
    ...
    it('two agents produce scopes with distinct observer IDs', () => {
        const http = makeMockHttp();
        const primeScope = new ConclusionScope(http, 'ws-test', 'agent-prime', 'owner');
        const devScope = new ConclusionScope(http, 'ws-test', 'agent-developer', 'owner');

...
Use honcho_analyze if you need Honcho to synthesize a complex answer.`,
    ...
    api.registerCli(
      ({ program, workspaceDir }) => {
        ...
        cmd
          .command("migrate")
          .description("Migrate existing memory files (USER.md, MEMORY.md, etc.) to Honcho")
          ...
          .action(async (options: { dryRun?: boolean }) => {
            ...
            function isAboutOwner(relativePath: string): boolean {
            ...
            async function collectFromDir(
              dirPath: string,
              relativePath: string,
              conclusions: Array<{ content: string; isAboutOwner: boolean }>
            ) {
            ...
            try {
              console.log("Scanning for memory files to migrate...");
              console.log(`Workspace: ${workspaceDir}`);
              console.log("");

              const conclusions: Array<{ content: string; isAboutOwner: boolean }> = [];
              ...
              if (conclusions.length === 0) {
                console.log("");
                console.log("No memory files found to migrate.");
                return;
              }
              ...
              for (const file of filesToDelete) {
                const targetPath = path.join(workspaceDir, file);
                await fs.promises.rm(targetPath, { force: true });
                console.log(`  Removed: ${file}`);
              }

              for (const dir of dirsToDelete) {
                const targetPath = path.join(workspaceDir, dir);
